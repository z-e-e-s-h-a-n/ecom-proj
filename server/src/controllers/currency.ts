...
export const getCurrencyByCountry = async (req: Request, res: Response) => {
  try {
    const { countryCode } = req.params;
    const currency = await CurrencyOptionModel.findOne({
      countryCode,
    });
    if (!currency)
      return sendResponse(res, 404, false, "Currency option not found.");
    sendResponse(res, 200, true, "Currency option retrieved.", currency);
  } catch (error) {
    sendResponse(res, 500, false, "Error fetching currency option.");
  }
};
